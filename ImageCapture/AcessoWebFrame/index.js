...
import AcessoWebFrameSupport from './AcessoWebFrameSupport';
import AcessoWebFramePopup from './AcessoWebFramePopup';
import AcessoWebFrameModel from './AcessoWebFrameModel';

let errorFragmentShader = false;

export default class AcessoWebFrame {
    constructor() {
    ...
    INTELLIGENT_CAMERA_FEEDBACK = Object.freeze({
      STAY_STILL: 'Não se mexa...',
      TOO_CLOSE: 'Afaste o rosto',
      TOO_FAR: 'Aproxime o rosto',
      OFF_CENTER: 'Centralize o rosto',
      UPWARDS: 'Rosto para cima',
      DOWNWARDS: 'Rosto para baixo',
      TILTED: 'Rosto inclinado',
      SIDEWAYS: 'Rosto de lado',
    });
    ...
    showButtonCapture = () => (this.buttonCapture.style.display = 'inline-block');

    hideButtonCapture = () => (this.buttonCapture.style.display = 'none');

    setButtonCaptureNormalPosition = () => {
    ...
    setButtonCaptureDocumentPosition = () => {
    ...
    calcBtnCapturePos = async() => {
        if (this.TYPE_PROCESS === this.TYPE_CAMERA.CAMERA_NORMAL) {
            this.setButtonCaptureNormalPosition();
            this.showButtonCapture();
        } else if (
    ...
    verifyFaceApiIsRunning = () => {
        if (this.isFaceApiIsRunning) {
            ...
            this.COLOR_SILHOUETTE.SECONDARY = this.COLOR_SILHOUETTE.WHITE;
            this.loadMask(this.COLOR_SILHOUETTE.SECONDARY);
            this.calcBtnCapturePos();
            this.acessoWebFramePopup.hideBoxLoading();
            this.acessoWebFramePopup.hideMessage();
    ...
    onPlay = async() => {
        try {
            if (this.TYPE_PROCESS === this.TYPE_CAMERA.CAMERA_NORMAL ||
                ...
                const options = this.acessoWebFrameModel.faceDetectorOptions;
                const ts = Date.now();
    ...
    isCentralizedFace = (leftX, leftY, rightX, rightY, noseX, noseY) => {
        ...
        if (leftX >= this.centralized.CSPWidthLeft - this.centralized.overSilhouetteThresholdHorizontal &&
            ...
            this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.STAY_STILL);
            ...
            if (rightX - leftX > this.centralized.silhoutteWidth) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.TOO_CLOSE);
            } else if (rightX - leftX < this.centralized.silhoutteWidth - this.centralized.inSilhouetteThresholdHorizontal) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.TOO_FAR);
            } else if (noseY <= this.centralized.CSPHeightTop) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.OFF_CENTER);
                ...
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.UPWARDS);
            } else if (rightX >= this.centralized.CSPWidthRight + this.centralized.overSilhouetteThresholdHorizontal) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.DOWNWARDS);
                ...
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.TILTED);
            } else if (this.centralized.differenceLeftY < -this.centralized.differenceNoseYThreshold && this.centralized.differenceRightY < -this.centralized.differenceNoseYThreshold) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.TILTED);
            } else if (this.centralized.distanceLeftByNose > this.centralized.distanceRightByNose) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.SIDEWAYS);
            } else if (this.centralized.distanceLeftByNose < this.centralized.distanceRightByNose) {
                this.acessoWebFramePopup.showMessage(this.INTELLIGENT_CAMERA_FEEDBACK.SIDEWAYS);
            }
    ...
    calcTopLabelMessagePosition = () => (this.cameraVideo.offsetHeight / 2 - this.mHeight / 2 - 25);

    setTopLabelMessage = () => {
        this.acessoWebFramePopup.Message.style.top = `${this.calcTopLabelMessagePosition()}px`;
    };

    browserNotSupportCallback = () => {
        return this.onBrowserNotSupportJS(this.acessoWebFrameSupport.getSupportedBrowsers());
    };
    ...
    hideBoxDocumentInfo = () => {
        let _box = this.boxCamera && this.boxCamera.querySelector('#box--document-info');
        if (_box) {
    ...
    initCameraNormal = (COLOR_SILHOUETTE_PRIMARY, FACE_MODE) => {
        if (!this.acessoWebFrameSupport.isSupportedBrowser) {
    ...
    initDocument = (TYPE, COLOR_SILHOUETTE_PRIMARY, LABEL_DOCUMENT_OTHERS) => {
        ...
        if (
            ...
            this.handleError('Tipo de documento inválido!');
        }
    };
}
